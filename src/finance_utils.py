...
from config import FINANCE_DATA_FILE
...
def load_finance_data(force=False):
    """读取 finance_data.json，带请求级缓存。
    
    返回: dict（完整 JSON），失败返回 None
    """
    import time
    now = time.time()
    ...
    _log(f"[finance] 读取 {FINANCE_DATA_FILE}")
    data = OneDriveIO.read_json(FINANCE_DATA_FILE)
    if data is None:
        _log("[finance] 读取失败")
        return None
    if not data:
...
def save_finance_data(data):
    ...
    ok = OneDriveIO.write_json(FINANCE_DATA_FILE, data)
    if ok:
        _finance_cache["data"] = data
        import time
...
def calc_snapshot_summary(items):
    ...
    返回: dict — {
        ...
        debt_ratio, by_asset_class, by_channel
    }
    """
    total_assets = 0.0
    ...
    for item in items:
        ...
        if cat == "资产":
            total_assets += amount
            if name in _ILLIQUID_NAMES or any(n in name for n in _ILLIQUID_NAMES):
    ...
    disposable = total_assets - illiquid_total - total_liabilities
    ...
    return {
        ...
        "disposable_assets": round(disposable, 2),
...
def compare_snapshots(current_items, previous_items):
    ...
    disp_chg, disp_pct = _change(curr["disposable_assets"], prev["disposable_assets"])
    ...
    return {
        ...
        "disposable_change": disp_chg,
        "disposable_change_pct": disp_pct,
