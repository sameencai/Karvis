...
from config import FINANCE_REPORTS_DIR
...
def execute(params, state):
    ...
    # 1. 先自动导入最新 iCost 数据
    import_result = _auto_import()

    # 2. 读取财务数据
    data = load_finance_data(force=True)
    if not data:
    ...
    # 10. 写入 Obsidian
    file_path = f"{FINANCE_REPORTS_DIR}/财务月报-{month_str}.md"
    ok = OneDriveIO.write_text(file_path, report_md)
...
def _auto_import():
    ...
    try:
        ...
        if result.get("success"):
            ...
            new_count = ctx.get("total_new_records", 0)
            if new_count > 0:
...
def _build_report_context(period_str, bill_summary, prev_summary,
    ...
    if snapshot_current:
        ctx["assets"] = snapshot_current["summary"]
        ctx["assets"]["snapshot_date"] = snapshot_current["date"]

    if snapshot_comparison:
        ctx["asset_comparison"] = {
...
def _build_report_markdown(period_str, month_str, bill_summary, prev_summary,
    ...
    if insights:
        ...
        if asset_health:
            ...
            if asset_health.get("risk_note"):
                lines.append(f"- **风险提示**：{asset_health['risk_note']}")
        ...
        # 财务目标进度（通用化，不绑定 FIRE）
        goal = insights.get("goal_progress") or insights.get("fire_progress")
        if goal and goal.get("progress_pct"):
            lines.extend(["## 🎯 财务目标进度", ""])
            lines.append("| 指标 | 数值 |")
            lines.append("|------|-----:|")
            if goal.get("annual_expense_estimate"):
                lines.append(f"| 年化支出 | {goal['annual_expense_estimate']} |")
            if goal.get("target_amount") or goal.get("fire_target"):
                target = goal.get("target_amount") or goal.get("fire_target")
                lines.append(f"| 目标金额 | {target} |")
            if goal.get("current_assets"):
                lines.append(f"| 当前资产 | {goal['current_assets']} |")
            if goal.get("progress_pct"):
                lines.append(f"| **进度** | **{goal['progress_pct']}** |")
            lines.append("")
            if goal.get("comment"):
                lines.append(goal["comment"])
                lines.append("")
...
def _build_wechat_summary(period_str, bill_summary, snapshot_current,
    ...
    # 财务目标进度
    goal = (insights.get("goal_progress") or insights.get("fire_progress", {})) if insights else {}
    if goal and goal.get("progress_pct"):
        parts.append(f"🎯 目标进度 {goal['progress_pct']}")
...
def handle_monthly(params, state):
...
# Skill 热加载注册表
SKILL_REGISTRY = {
    "finance.monthly": handle_monthly,
}
