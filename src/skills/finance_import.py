...
触发方式：
  - 手动：用户说「导入 iCost 数据」
  - 自动：月报生成前自动调用（Phase 3）
"""
import sys
import hashlib
import io
from config import FINANCE_INBOX_DIR
...
def _bill_hash(bill):
    ...
    与财务管理助手 v4.0 的 getBillHash 保持一致。
    """
    parts = [
        str(bill.get("日期", "")),
        str(bill.get("金额", "")),
...
def _scan_and_import():
    ...
    """
    # 1. 列出 inbox 目录
    items = OneDriveIO.list_children(FINANCE_INBOX_DIR)
    if items is None:
    ...
    for item in xlsx_files:
        ...
        # 下载文件
        file_path = f"{FINANCE_INBOX_DIR}/{fname}"
        file_bytes = OneDriveIO.download_binary(file_path)
...
def handle_import(params, state):
    """iCost 数据导入入口。
