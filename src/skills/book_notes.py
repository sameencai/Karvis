...
def _book_file(name):
    return f"{BOOK_NOTES_DIR}/{name}.md"
...
"""
        ...
        # 更新书单索引
        _update_book_list(name, author, category)
        ...
        if first_thought:
            ...
            thought_result = thought({"content": first_thought, "book": name}, state)
...
def excerpt(params, state):
    ...
    entry = f"> {content}\n*— {_now_str()}*\n"
    ok = OneDriveIO.append_to_section(_book_file(book), "## ✂️ 摘录", entry)
...
def thought(params, state):
    ...
    entry = f"{content}\n*— {_now_str()}*\n"
    ok = OneDriveIO.append_to_section(_book_file(book), "## 💡 我的思考", entry)
...
def summary(params, state):
    ...
    content = OneDriveIO.read_text(_book_file(book))
...
*🤖 AI 生成于 {_now_str()}*
"""
    ok = OneDriveIO.append_to_section(_book_file(book), "## 🤖 AI 总结", summary_md)
...
def quotes(params, state):
    ...
    quotes_md += f"\n\n*🤖 AI 提炼于 {_now_str()}*\n"
    OneDriveIO.append_to_section(_book_file(book), "## 💎 可分享的金句", quotes_md)
...
def _update_book_list(name, author, category):
    """更新书单索引"""
    existing = OneDriveIO.read_text(BOOK_LIST_FILE) or ""
    if not existing.strip():
        existing = "# 📚 书单\n\n| 书名 | 作者 | 分类 | 状态 | 日期 |\n|------|------|------|------|------|\n"
    ...
    OneDriveIO.write_text(BOOK_LIST_FILE, new_content)
