...
def handle_snapshot(params, state):
    """
    资产快照查询入口。

    params:
        query_type: summary|compare|by_category|by_channel|trend
        category: str — 按 subCategory 筛选（如"投资理财"、"流动资金"）
        channel: str — 按 channel 筛选（如"招商银行"、"富途"）
    ...
    if query_type == "summary":
        ...
        _log(f"[finance.snapshot] summary | date={latest_date} | "
             f"assets={summary['total_assets']} disp={summary['disposable_assets']}")
        return {"success": True, "agent_context": ctx}
    ...
    if query_type == "trend":
        ...
        for d in dates[:6]:
            ...
            trend_data.append({
                ...
                "disposable_assets": s["disposable_assets"],
...
# Skill 热加载注册表
SKILL_REGISTRY = {
    "finance.snapshot": handle_snapshot,
}
