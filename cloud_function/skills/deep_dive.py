...
def dive(params, state):
    ...
    # 1. 搜索全历史数据
    raw_data = _collect_data(keywords, state)
...
def _collect_data(keywords, state):
    """搜索全历史数据，返回结构化结果"""
    from config import (
    ...
    files_to_read["memory"] = MEMORY_FILE
...
def _save_report(topic, report):
    ...
    file_path = f"{OBSIDIAN_BASE}/02-Notes/深潜报告/{date_str}-{safe_topic}.md"
