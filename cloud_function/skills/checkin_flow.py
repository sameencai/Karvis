...
def answer(params, state):
    ...
    if step < 4:
        ...
        reply = finish(state, timeout=False)
        return {
            "success": True,
            "reply": reply,
            "state_updates": {
...
def finish(state, timeout=False):
    """
    ...
    # 写入 Daily Note
    daily_note_path = f"{DAILY_NOTES_DIR}/{checkin_date}.md"
