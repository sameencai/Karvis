...
from config import (
    QUICK_NOTES_FILE, EMOTION_NOTES_DIR,
    WORK_NOTES_DIR, FUN_NOTES_DIR, MISC_FILE,
    DAILY_NOTES_DIR, DECISION_LOG_FILE
)
...
def execute(params, state):
    ...
    # 1. 并发收集当天所有数据
    data = _collect_mood_data(date_str, state)
    ...
    file_path = f"{EMOTION_NOTES_DIR}/{date_str}.md"
...
def _collect_mood_data(date_str, state):
    """并发收集当天所有情绪相关数据"""
    from concurrent.futures import ThreadPoolExecutor

    files_to_read = {
        "quick_notes": QUICK_NOTES_FILE,
        "emotion": f"{EMOTION_NOTES_DIR}/{date_str}.md",
        "fun": f"{FUN_NOTES_DIR}/{date_str}.md",
        "work": f"{WORK_NOTES_DIR}/{date_str}.md",
        "misc": MISC_FILE,
        "daily": f"{DAILY_NOTES_DIR}/{date_str}.md",
