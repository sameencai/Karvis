...
def execute(params, state):
    ...
    # 1. 并发收集整月数据
    data = _collect_month_data(dates, month_str, state)
    ...
    # 4. 写入文件
    file_path = f"{DAILY_NOTES_DIR}/月报-{month_str}.md"
...
def _collect_month_data(dates, month_str, state):
    """并发收集整月数据"""
    ...
    for wd in week_dates:
        files_to_read[f"weekly_{wd}"] = f"{DAILY_NOTES_DIR}/周报-{wd}.md"
